...
@section('mainContent')
    <form class="insert-update-row-form" id="categories-form" onsubmit="checkCategory()" action="{{route('categories.update',['id'=>$data['category']['id']])}}" method="POST">
        @csrf
        @method('put')
        ...
        <select  name="select-category" id="select-category">
            ...
            @foreach($data['parentCategories'] as $category)
                <option value="{{$category['id']}}" @if($data['category']->parent && ($category['id'] == $data['category']->parent['id']))selected @endif>{{$category['name']}}</option>
            @endforeach
        </select>

        <label for="category-name" >Name</label>
        <input type="text" value="{{$data['category']['name']}}" id="category-name" name="category-name">
